...
const FlashcardSetForm = () => {
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-xl font-bold mb-4">Create Flashcard Set</h1>
      <form onSubmit={handleSubmit}>
        ...
        {cards.map((card, index) => (
          <div key={index} className="mb-2 border p-2">
            <input className="border p-1 w-full mb-1" placeholder="Term" value={card.term} onChange={(e) => handleCardChange(index, 'term', e.target.value)} />
            <input className="border p-1 w-full" placeholder="Definition" value={card.definition} onChange={(e) => handleCardChange(index, 'definition', e.target.value)} />
